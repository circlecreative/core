<div class="nk-block-head">
    <div class="nk-block-between-md g-3">
        ...
        <div class="nk-block-head-content">
            <a href="{{ route('profile.activity.clear') }}" class="link link-danger clear-profile-log" data-action="ajax">{{ __('Clear log') }}</a>
        </div>
    </div>
</div>
@if($activities)
            ...
            @forelse($activities as $activity)
                <tr>
                    ...
                    <td class="tb-col-time">
                        <span class="sub-text">{{ $activity->session }}  <span class="d-none d-sm-inline-block">{{ $activity->session }}</span></span>
                    </td>
                    <td class="tb-col-action">
                        @if(!$loop->first)
                            <a href="{{ route('profile.activity.delete', ["id" => $activity->id]) }}" class="link-cross mr-sm-n1 clear-profile-log" data-action="ajax"><em class="icon ni ni-cross"></em></a>
                        @endif
                    </td>
                </tr>
            @empty
...
@endif
@if(__('last_clear_activity'))
<div class="notes mt-3 font-italic text-soft small">
    <p>{{  __('Last cleared log at :date') }}</p>
</div>
@endif
