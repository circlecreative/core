@extends('layouts.app')
@section('content')
<form id="form-post" class="nk-content-inner" action="{{ route('system.tags.update',['tag'=>$tag->id]) }}" method="post" enctype="multipart/form-data">
@csrf
@method('PUT')
    ...
    <div class="nk-block-des text-soft text-sm">
        <ul class="list-inline">
            <li class="text-muted">
                <small>ID: <span class="font-weight-bold">{{$tag->id}}</span></small>
                <input type="hidden" name="id" value="40">
            </li>
            <li class="text-muted">
                <small>Last Update: <span class="font-weight-bold">{{$tag->updated_at}}</span></small>
            </li>
        </ul>
    </div>
                ...
                <div class="col-lg-7">
                    <div class="form-group">
                        <div class="form-control-wrap">
                            <input type="text" class="form-control" id="input-title" value="{{$tag->title ?? null}}" name="title">
                        </div>
                    </div>
                </div>
            </div>
